package core;
import java.io.*;
import static core.GeneralUtils.*;
...
public class FileUtilManager {
    ...
    public FileUtilManager(String _path, String operatingSystem) {
        path = _path;
        os = operatingSystem;
    }
    ...
    // "C:/Users/trent" will not be included so it will look something like this... "Desktop/core.Ion"
    ...
    private void InstantiateCSSOutput() {
        this.outputPath = this.GetPathDirectory() + (this.os.equals("win") ? "\\" : "/") + this.TruncateExtension(this.GetFileName()) + ".css";
        ...
        try(BufferedWriter writer = new BufferedWriter(new FileWriter(this.outputPath))) {
            writer.write("/* Generated by core.Ion */");
            writer.flush();
        } catch(Exception err) {
            err.printStackTrace();
        }
